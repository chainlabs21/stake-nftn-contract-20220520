...
library SafeTransferLib {
    function safeTransferETH(address to, uint256 amount) internal {
        bool callStatus;
        assembly {            // Transfer the ETH and store if it succeeded or not.
...
contract StakingContractMainnet {
    ...
    function createIncentive(
        ...
        uint32 startTime, // 0 
        uint32 endTime // 1000
				, uint256 _timerefpoint
    ) external returns (uint256 incentiveId) {
        if (startTime < _timerefpoint ) startTime = uint32( _timerefpoint );
        ...
        incentives[incentiveId ] = Incentive({
            ...
            liquidityStaked: 0,            // Initial value of rewardPerLiquidity can be arbitrarily set to a non-zero value.
		...
		function updateIncentive_with_timerefpoint (
			uint256 incentiveId,
			int112 changeAmount,
			uint32 newStartTime,
			uint32 newEndTime , 
			...
			_accrueRewards(incentive , _timerefpoint );
			if (newStartTime != 0) {
					if (newStartTime < _timerefpoint ) newStartTime = uint32( _timerefpoint );
					incentive.lastRewardTime = newStartTime;
			}
			if (newEndTime != 0) {
					if (newEndTime < _timerefpoint ) newEndTime = uint32( _timerefpoint );
    ...
    function stakeAndSubscribeToIncentives (
        ...
        stakeToken(token, amount, transferExistingRewards , _timerefpoint );
        uint256 n = incentiveIds.length;
        for (uint256 i = 0; i < n; i = _increment(i)) {
            subscribeToIncentive(incentiveIds[i] , _timerefpoint );
        }
    }
    function stakeToken(address token, uint112 amount, bool transferExistingRewards
			, uint256 _timerefpoint
		) public {
    ...
    function unstakeToken(address token, uint112 amount, bool transferExistingRewards
			, uint256 _timerefpoint
		) external {
        UserStake storage userStake = userStakes[msg.sender][token];
        uint112 previousLiquidity = userStake.liquidity;
    ...
    function subscribeToIncentive(uint256 incentiveId , uint256 _timerefpoint ) public {
    ...
    function accrueRewards(uint256 incentiveId , uint256 _timerefpoint )  external {
        _accrueRewards(incentives[incentiveId] , _timerefpoint );
    }
    function claimRewards(uint256[] calldata incentiveIds , uint256 _timerefpoint ) external returns (uint256[] memory rewards) {
			uint256 n = incentiveIds.length;
    ...
    function _accrueRewards ( Incentive storage incentive , uint256 _timerefpoint ) internal {
			unchecked {
				uint256 maxTime = _timerefpoint < incentive.endTime ? _timerefpoint : incentive.endTime;
